...
- /tasks/today           (today task list + summary)
- /tasks/{task_id}/complete
- /tasks/{task_id}/uncomplete
- /tasks/cleanup
- create_daily_tasks (helper)
"""
...
# ============================================
# PYDANTIC MODELS (single response shape)
# ============================================
...
def build_mock_todays_tasks(student_id: str = "demo", message: str = "Mock data") -> TodaysTasksResponse:
    """Unified mock response (typed, consistent)"""
...
def group_tests_by_topic(topic_tests: List[Dict[str, Any]]) -> Dict[str, Dict[str, Any]]:
    """
    Groups student_topic_tests by topic_id and returns:
    {
      topic_id: { topic_name, subject_name, tests: [...] }
    }
    """
    topic_performance: Dict[str, Dict[str, Any]] = {}

    for test in topic_tests:
        # Expecting join: topics(name_tr, subjects(name_tr))
...
def calculate_at_risk_topics(topic_performance: Dict[str, Dict[str, Any]], limit: int = 3) -> List[TopicAtRisk]:
    """
    ✅ SINGLE SOURCE OF TRUTH: At-risk calculation
    Uses:
      - calculate_remembering_rate(tests)
      - calculate_next_review_date(rate, last_test_date)
    """
    ...
    for topic_id, data in topic_performance.items():
        ...
        if not tests:
            print(f"⏭️  {data.get('topic_name', 'Unknown')}: No tests, skipping")
            continue

        latest = tests[0]
        latest_date_raw = latest.get("test_date", "")
        if not latest_date_raw:
            print(f"⏭️  {data.get('topic_name', 'Unknown')}: No test_date, skipping")
            continue
    ...
    print(f"\n📊 SUMMARY: {len(at_risk)} at-risk topics found, returning {len(at_risk[:limit])}")
    print(f"=== END ===\n")
...
def calculate_priority_topics(topic_performance: Dict[str, Dict[str, Any]], limit: int = 3) -> List[PriorityTopic]:
    """
    Priority topics based on avg success of last up to 3 tests.
    """
    ...
    for topic_id, data in topic_performance.items():
        ...
        denom = max(1, len(success_values))
        avg_success = sum(success_values) / denom
...
def calculate_streak(all_tests: List[Dict[str, Any]], x_user_timezone: str) -> StudyStreak:
    """
    Simple streak: consecutive days with at least one test entry.
    """
    ...
    return StudyStreak(
        ...
        longest_streak=max(current_streak, 12),  # placeholder, later compute from history
...
async def get_todays_tasks(
    ...
    try:
        ...
        # Placeholder time stats (later from real study sessions)
        time_stats = TimeStats(
        ...
        return build_mock_todays_tasks(student_id=student_id, message="Fallback mock (error)")
...
async def get_todays_tasks_list(
    ...
    """
    Bugünün görev listesi + özet + at-risk (aynı motor)
    """
    try:
        student_id = current_user["id"]
        supabase = get_supabase_admin()

        # NOTE: task_date should match DB format. Here we use ISO date string "YYYY-MM-DD"
        ...
        # Convert to dict for this endpoint (if you want typed here too, we can)
...
async def complete_task(
    ...
    manual=False -> otomatik (test girişi sonrası)
    """
    try:
        supabase = get_supabase_admin()
...
# ============================================
# TASK CREATION (single definition)
# ============================================

def create_daily_tasks(student_id: str, date: str):
    """
    4 Motor'dan günlük görevler oluştur (gerçek topic_id'lerle)
    NOTE: Şimdilik demo veriler; Batch 2'de motorlardan gerçek öneriler gelecek.
    """
    supabase = get_supabase_admin()
