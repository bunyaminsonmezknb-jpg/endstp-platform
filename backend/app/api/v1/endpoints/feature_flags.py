...
async def toggle_flag(flag_key: str, request: ToggleFlagRequest):
    """Toggle feature flag on/off"""
    supabase = get_supabase_admin()

    # ✅ READ from write-table (audit trigger burada)
    flag_result = (
        supabase.table("feature_flags_monitoring")
        .select("*")
    ...
    # ✅ WRITE to write-table
    supabase.table("feature_flags_monitoring").update(update_data).eq("flag_key", flag_key).execute()

    print(f"🎛️  Flag toggled: {flag_key} → {new_state}")

    return {"success": True, "flag_key": flag_key, "is_enabled": new_state}


# ------------------------------------------------------------
# Lite error reporting (eski sürüm) -> path değiştirildi
# ------------------------------------------------------------
@router.post("/{flag_key}/report-error-lite")
async def report_flag_error_lite(flag_key: str, request: ReportErrorRequest):
    """Enhanced error reporting with runtime metrics (LITE)"""
    supabase = get_supabase_admin()
    ...
    new_error_count = (flag.get("error_count") or 0) + 1
...
async def get_health_status():
    ...
    result = supabase.table("feature_flags_monitoring").select("*").execute()
    ...
    critical_flags = [
        ...
        for f in flags
        if f.get("error_severity") in ["critical", "high"] or f.get("health_score", 100) < 50
    ]
...
async def quick_action(flag_key: str, action: str):
    ...
    if action == "reset_health":
        ...
        current_attempts = flag.get("recovery_attempts", 0) or 0
        update_data.update(
            {
...
# Enhanced error reporting (final) -> aynı path korunuyor
@router.post("/{flag_key}/report-error")
async def report_flag_error(flag_key: str, request: ReportErrorRequest):
    """Enhanced error reporting with runtime metrics & health breakdown (FINAL)"""
    supabase = get_supabase_admin()
